...
const ParentSpace = () => {
    ...
    // On affiche dans "Panier" : REQUEST + PENDING
    const requestedItems = useMemo(
        () => items.filter((it) => it.status === "REQUEST"),
        [items]
    );
    ...
    /**
     * Bouton OK :
     * - Si item REQUEST : le parent valide la demande => REQUEST -> PENDING
     * - Si item PENDING : on ne fait rien ici (le paiement "Payer maintenant" gère l'ORDER)
     */
    const handleValidateItem = async (item: ParentCartItem) => {
        ...
        try {
            if (item.status === "REQUEST") {
                await approveCartRequest(cartItemId);
                await loadCart();
                return;
            }

            // Cas PENDING : on ne "commande" pas au clic OK dans la liste.
            // Le parent finalise via "Payer maintenant".
            await loadCart();
        } catch (err) {
            console.error(err);
            setError("Impossible de valider la demande.");
        } finally {
    ...
    /**
     * Payer maintenant
     * - valide tous les items visibles (REQUEST + PENDING) via /order (un par un)
    ...
    const renderItem = (item: ParentCartItem) => {
        ...
        const statusLabel = item.status === "REQUEST" ? "Demandé" : "Dans le panier";
        const statusBg =
            item.status === "REQUEST"
        ...
        const okTitle = item.status === "REQUEST" ? "Valider la demande" : "Déjà dans le panier";
                ...
                {statusLabel}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                type="button"
                                onClick={() => void handleValidateItem(item)}
